...
class SolisExporter:
    def __init__(self, config_path: str) -> None:
        ...
        self.bind_host = str(server.get("bind_host", server.get("host", "0.0.0.0"))).strip() or "0.0.0.0"
        self.port = int(server.get("port", 9121))
        ...
        logging.info(
            "solis-exporter bind=%s port=%s inverters=%s interval=%ss timeout=%ss retries=%s parallel=%s stale=%ss default_metrics=%s",
            self.bind_host,
            self.port,
            len(self.inverters),
    ...
    def run(self) -> None:
        ...
        httpd = make_server(self.bind_host, self.port, self.wsgi_app(), server_class=ThreadingWSGIServer)
        
        logging.info("serving on http://%s:%s (metrics=/metrics)", self.bind_host, self.port)
        httpd.serve_forever()
